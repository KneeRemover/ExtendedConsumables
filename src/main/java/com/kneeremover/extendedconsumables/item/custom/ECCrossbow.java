...
public class ECCrossbow extends ProjectileWeaponItem implements Vanishable {
	...
	private static void shootProjectile(Level pLevel, LivingEntity pShooter, InteractionHand pHand, ItemStack pCrossbowStack, ItemStack pAmmoStack, float pSoundPitch, boolean pIsCreativeMode, float pVelocity, float pInaccuracy, float pProjectileAngle) {
		if (!pLevel.isClientSide) {
			boolean IsFireworkRocket = pAmmoStack.is(Items.FIREWORK_ROCKET);
			boolean isCustomArrows = pAmmoStack.is(ModItems.TIPPED_BOLT_ITEM.get());
			...
			} else if (isCustomArrows) {
				projectile = getTippedArrow(pLevel, pShooter, pCrossbowStack, pAmmoStack);
	...
	private static TippedBolt getTippedArrow(Level pLevel, LivingEntity pLivingEntity, ItemStack pCrossbowStack, ItemStack pAmmoStack) {
		TippedBoltItem arrowitem = (TippedBoltItem) (pAmmoStack.getItem() instanceof TippedBoltItem ? pAmmoStack.getItem() : ModItems.TIPPED_BOLT_ITEM.get().getDefaultInstance());
		TippedBolt arrow = arrowitem.createArrow(pLevel, pAmmoStack, pLivingEntity);
		if (pLivingEntity instanceof Player) {
	...
	public static void performShooting(Level pLevel, LivingEntity pShooter, InteractionHand pUsedHand, ItemStack pCrossbowStack, float pVelocity, float pInaccuracy) {
		if (pShooter instanceof Player player && net.minecraftforge.event.ForgeEventFactory.onArrowLoose(pCrossbowStack, pLevel, player, 1, true) < 0)
			return;
